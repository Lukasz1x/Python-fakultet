...
class MainWidget(QWidget):  # dziedziczenie po QWidget
    def __init__(self):  # konstruktor
        ...
        self.city_list.itemDoubleClicked.connect(self.__move_city_to_fav)       # po kliknięciu na miasto z listy wyników wywołujemy metodę _on_city_clicked()
    ...
    def __get_weather_for_clicked_city(self):
        ...
        latitude, longitude = self.fav_city_list.currentItem().get_geo_params()
        params = []
        for key, val in self.weather_params.items():
            if val == True:
                params.append(key)
        response = requests.get(f'https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&current={','.join(params)}')  # wysyłamy zapytanie do api z pobranymi współrzędnymi
        json = response.json()  # tworzymy słownik
