...
class MainWidget(QWidget):  # dziedziczenie po QWidget
    def __init__(self):  # konstruktor
        ...
        self.button.clicked.connect(
            self._on_button_clicked)  # po kliknięciu przycisku wywołujemy metodę _on_button_clicked() - connect przyjmuje metodę bez ()
        self.city_list.itemClicked.connect(
    ...
    def _on_button_clicked(self):
        ...
        response = requests.get(
            f'https://geocoding-api.open-meteo.com/v1/search?name={text}')  # wysyłamy zapytanie do api pogodowego
        # print(response.json())
        json = response.json()  # tworzymy słownik
        if "results" not in json.keys():  # jeśli nie ma wyników
            QMessageBox.information(self, "Błąd",
                                    "Nie ma takiej miejscowości.")  # wyskakuje okienko z informacją o błędzie
            return
    ...
    def _on_city_clicked(self):
        latitude, longitude = self.city_list.currentItem().data(
            Qt.UserRole)  # pobieramy dane z aktualnie wybranego elementu
        response = requests.get(
            f'https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&current=temperature_2m')  # wysyłamy zapytanie do api z pobranymi współrzędnymi
        json = response.json()  # tworzymy słownik
        self.label.setText(
            f"{json["current"]["temperature_2m"]} stopni C")  # pobieramy z sekcji current temperaturę i wyświetlamy