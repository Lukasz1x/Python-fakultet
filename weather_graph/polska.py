...
class PolandMap:
    def __init__(self, shapefile_url=None):
        ...
        self._world = gpd.read_file(self.shapefile_url)                             # wybranie tylko danych dla polski
        self.poland = self._world[self._world['ADMIN'] == 'Poland']                 # indeks aktywnego miasta do wykresu temperatury (0 = Warszawa)
        ...
        def draw_cities(self, ax, data, fig):
            ...
            plot = ax.scatter(x, y, c=c,
                              cmap="Spectral_r")                                    # metoda scatter tworzy wykres punktowy i mapuje temperatury na kolor punktów
        ...
        def draw_cities_labels(self, ax, data):
            for city, entry in zip(cities, data):                                   # łącznie nazwy miasta i danych o temperaturze
                ...
                ax.text(city[2], city[1], label, ha="center",
                        bbox=dict(boxstyle="Round,pad=0.2", fc="white", alpha=0.2)) # narysowanie etykiety na wykresie
        ...
        def get_data(self):
            ...
            latitude = ",".join([str(city[1]) for city in
                                 cities])                                           # tworzy ciągi z szerokościami i długościami geograficznymi wszystkich miast (oddzielone przecinkami)
        ...
        def draw_temperature_plot(self, ax, data):
            ...
            hourly = data[self.active_index][
                "hourly"]                                                           # pobranie danych godzinowych temperatury dla aktualnie wybranego miasta (active_index)
            hours = [datetime.strptime(i, format) for i in
                     hourly["time"]]                                                # konwersja ciągów znaków z godzinami na obiekty datetime dla osi X wykresu
            ax.plot(hours, hourly["temperature_2m"], label="temperatura",
                    color="red")                                                    # rysowanie wykresu temperatury w czasie
        ...
        def onclick(self, event):
            if event.inaxes == self.map_ax:                                         # sprawdzenie, czy kliknięcie nastąpiło w obrębie mapy
                for i, city in enumerate(
                        cities):                                                    # iteracja po wszystkich miastach, by sprawdzić, czy kliknięto w obrębie któregoś z nich
                    ...
                    if math.hypot(city[2] - event.xdata, city[1] - event.ydata) < 0.25:  # obliczenie odległości między punktem kliknięcia a pozycją miasta
                        ...
                        self.draw_temperature_plot(self.temp_ax,
                                                   self.data)                       # ponowne rysowanie nowego wykresu temperatury
                    print(
                        '%s click: button=%d, x=%d, y=%d, xdata=%f, ydata=%f' %     # wypisanie danych do kliknięciu do konsoli
